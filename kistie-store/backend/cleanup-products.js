...
// - For products with missing images, assign a random valid image
// - Only keep products whose images exist in the images folder
...
// Images to exclude from products
const excluded = [
  ...
  db.all('SELECT id, image FROM products', [], (err, rows) => {
    ...
    rows.forEach(row => {
      ...
      if (!validImages.includes(imgFile)) {
        ...
        db.run('UPDATE products SET image = ? WHERE id = ?', [`images/${randomImg}`, row.id]);
      }
    });
  });
